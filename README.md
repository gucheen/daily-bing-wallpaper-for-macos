...
- 下载失败保留上次壁纸，10 分钟后重试。
- 更新时接受本地当天及以后日期的壁纸，提前上线的新照片正常采用，且不会回退到更早日期。上游仍是过期日期或过期照片时保留当前壁纸，10 分钟后重试。当日或未来日期的相同照片复用缓存，链接变化时额外比较原图内容。
...
测试覆盖每日更新时间、深色图生成、原图保留、缓存读写、损坏图片拒绝、日期校验、提前上线壁纸采用与复用、防止日期回退、重复照片识别及缓存缺失修复。
